...
test('presents the research question and an honest recorded fallback', async () => {
  ...
  expect(screen.getByLabelText('1:3 hybrid routing diagram: 4 attention layers and 12 Mamba-2 layers')).toBeInTheDocument()
  expect(document.querySelectorAll('.routing-pattern i')).toHaveLength(16)
  expect(document.querySelectorAll('.routing-pattern i.attention')).toHaveLength(4)
...
test('ratio selection updates the shared architecture readout', async () => {
  ...
  expect(screen.getByLabelText('1:15 hybrid routing diagram: 1 attention layer and 15 Mamba-2 layers')).toBeInTheDocument()
  expect(document.querySelectorAll('.routing-pattern i.attention')).toHaveLength(1)
