...
test('desktop reader reaches evidence before the optional demo', async ({ page }) => {
  await page.goto('/')
  ...
  await expect(page.locator('.demo-details')).not.toHaveAttribute('open', '')
...
test('replay remains explicitly recorded and refuses an unmeasured prompt', async ({ page }) => {
  await page.goto('/')
  await page.getByText('Open generation instrument').click()
